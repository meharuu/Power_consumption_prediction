...
| | |
|---|---|
| **Task** | Forecast hourly power consumption (kWh) for 100 buildings of 10 types |
| **Competition** | Dacon AI Big Data Competition (sponsored by the Korea Energy Agency) |
| **Metric** | SMAPE (lower is better) |
| **Final score** | **7.50% SMAPE** on the competition test set |
| **Approach** | Per-building EDA → ~30 engineered features → per-building outlier clipping → gradient-boosted trees |
...
LightGBM, XGBoost, CatBoost, and Random Forest were each trained and compared. In the
per-building analysis (`bldgs.ipynb`), models were validated with **`TimeSeriesSplit`**,
which always trains on the past and validates on the future. The modelling notebooks use
quicker random hold-out splits to iterate, so their internal validation scores are
optimistic; the **7.50% SMAPE** is the score on the competition's held-out test period,
which the models never saw.

Hyperparameters were tuned with `GridSearchCV` (`new2.ipynb`). Gradient-boosted trees dominate here for the
...
- Switch every modelling notebook to `TimeSeriesSplit`, so internal validation matches the leaderboard
- Consolidate the notebooks into a single pipeline (`features.py`, `train.py`) with one config per building type
- Blend the per-type models with the global model, weighted by per-building validation SMAPE
